...
export class BookService {
    ...
    async findBook (whereFind, searchWord) : Promise<object[]> {
        ...
        switch (whereFind) {
            case "Author" : {
                foundBook = await this.bookClass.searchMatches('name_author', searchWord);
                break;
            }
            case "Book's name" : {
            ...
            case "ID's book" : {
                const book = await this.bookRepository.findOne({ where : { id_book : searchWord, status : 'Checked' }})
                if (book) foundBook.push(book)
                break;
            }
            case "Genre" : {
                foundBook = await this.bookRepository.find({ where : { genre : searchWord, status : 'Checked' }})
                break;
            }
        }                        
